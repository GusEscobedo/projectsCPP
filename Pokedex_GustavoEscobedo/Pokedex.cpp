...
public:
	...
	void AddPookemon();
	void Searching(const POOkedex& aPOOkedex);
private:
	POOmon* m_pHead;
};
...
void POOkedex::Searching(const POOkedex& aPOOkedex) {

	string search;
	bool found = true;
	POOmon* pIter = aPOOkedex.m_pHead;

	cout << "Search for my Pookemon: ";
	cin >> search;
	
	while (found == true) {
		if (search == pIter->GetName())
		{
			cout << "Name: " << pIter->GetName() << "\nType: " << pIter->GetType() << "\nAge: " << pIter->GetAge();
			found = false;
		}
		pIter = pIter->GetNext();
	}
...
int main()
{
	...
	do
	{
		...
		case 3: myPookedex.Searching(myPookedex); break;
